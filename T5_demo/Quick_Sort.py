...
class Quick():
    def Quick_sort(self, unsorted, l, r, reverse=False, progress_bar=None):
        '''
            i表示[l,i]都比pivot小
            j依次遍历元素
        '''
        ...
        # 防止空列表或无效索引
        if not unsorted or l < 0 or r >= len(unsorted):
            return 0
            
        # 估计排序操作次数（快速排序平均情况下的比较次数约为 n*log(n)）
        n = r - l + 1
        total_operations = int(n * math.log2(n) if n > 1 else 1)
        ...
        while stack:
            ...
            # 安全检查
            if high >= len(unsorted):
                continue
                
            # i 初始值是-1
            pivot, i = unsorted[high], low - 1
            
            # 分区操作
            for j in range(low, high):
                # 如果小于pivot， 则交换，交换的目的是保证[l,i]都比pivot小
                if cmp(pivot, unsorted[j]) == reverse:
                    i += 1
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="藏文快速排序命令行工具")
    ...
    with tqdm(total=100, desc="排序进度", unit="%") as pbar:
        quick = Quick()
        sort_time = quick.Quick_sort(words, 0, len(words)-1, reverse_flag, pbar)
    
    print(f"快速排序完成，用时{sort_time:.2f}秒")
...
# 执行命令
# python T5_demo/Quick_Sort.py input.txt output.txt --reverse true --preview
