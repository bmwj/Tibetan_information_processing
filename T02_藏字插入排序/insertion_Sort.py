...
class Insertion():
    def insertion_sort(self, arr, reverse=False, progress_bar=None):
        """插入排序主函数"""
        timA = time.time()
        ...
        for i in range(1, len(arr)):
            ...
            if progress_bar is not None and total_operations > 0:
                ...
                if progress_percent > last_percent:
                    progress_bar.update(progress_percent - last_percent)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="藏文插入排序命令行工具")
    ...
    with tqdm(total=100, desc="排序进度", unit="%") as pbar:
        insertion = Insertion()
        sort_time = insertion.insertion_sort(words, reverse=reverse_flag, progress_bar=pbar)
    
    print(f"插入排序完成，用时{sort_time:.2f}秒")
...
# 执行命令
# python T2_demo/insertion_Sort.py input.txt output.txt --reverse true --preview
